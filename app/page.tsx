...
function CompactPaperArticle({ paper }: { paper: Extract<RenderPaper, { kind: "compact" }> }) {
  return (
    <article className="paper-article" id={paper.id}>
      ...
      <section className="paper-section conclusion-section">
        <h4>先给结论</h4>
      ...
      <section className="paper-section relation-section">
        <h4>和研究主线的关系</h4>
        <p>{paper.relation}</p>
      </section>
    </article>
  ...
  return (
    <article className="paper-article" id={paper.id}>
      ...
      {intro.length > 0 ? (
        <section className="paper-section conclusion-section">
          <h4>先给结论</h4>
          {intro.map((section) => <SectionBody key={`${paper.id}-${section.heading}`} section={section} />)}
        </section>
      ) : null}
      ...
      {background.length > 0 ? (
        <section className="paper-section">
          <h4>读懂它需要的最少背景</h4>
          {background.map((section) => <SectionBody key={`${paper.id}-${section.heading}`} section={section} />)}
        </section>
      ) : null}
      ...
      {understanding.length > 0 ? (
        <section className="paper-section three-minute">
          <h4>3 分钟理解</h4>
          {understanding.map((section) => (
            <div className="understanding-block" key={`${paper.id}-${section.heading}`}>
              {section.heading !== "3 分钟理解" ? <h5>{section.heading}</h5> : null}
      ...
      {evidence.length > 0 || limits.length > 0 ? (
        <section className="paper-section evidence-grid">
          <div><h4>实验证据</h4>{evidence.map((section) => <SectionBody key={`${paper.id}-${section.heading}`} section={section} />)}</div>
          <div><h4>局限</h4>{limits.map((section) => <SectionBody key={`${paper.id}-${section.heading}`} section={section} />)}</div>
        </section>
      ) : null}
      ...
      {relation.length > 0 ? (
        <section className="paper-section relation-section">
          <h4>和研究主线的关系</h4>
          {relation.map((section) => <SectionBody key={`${paper.id}-${section.heading}`} section={section} />)}
        </section>
      ) : null}

      {remaining.map((section) => (
        <section className="paper-section" key={`${paper.id}-${section.heading}`}>
...
function paperSearchText(paper: RenderPaper) {
  if (paper.kind === "compact") {
    return [paper.title, paper.zhTitle, paper.arxiv, paper.track, paper.secondary, paper.conclusion, paper.background, paper.flow.join(" "), paper.evidence, paper.relation].filter(Boolean).join(" ");
  }
...
export default function Home() {
  ...
  const days = useMemo<RenderDay[]>(() => {
    const early = (earlyArchive as { days: CompactDay[] }).days.map<RenderDay>((day) => ({
      ...day,
    ...
    const later = (laterPlan as { days: PlanDay[] }).days.map<RenderDay>((day) => ({
      ...
      papers: day.papers.map((planned) => {
        ...
        return {
          ...
          id: `fallback-${planned.arxiv.replace(".", "-")}`,
          ...planned,
          conclusion: "这篇论文属于当天历史简报的主线。旧网站数据源没有正确恢复其完整正文，因此当前页面保留经过核对的论文、日期和研究方向，并以当日研究信号说明其位置。",
          background: "该条目用于防止旧网站因按论文提交日期归档而漏掉当天实际阅读的论文。后续内容仍以原论文和历史精读为依据。",
          flow: ["论文身份和 arXiv 编号已经按当天历史记录恢复。", "当前页面不再根据论文提交日期自动决定阅读日期。", "研究方向和当天横向关系已经恢复。"],
          evidence: "论文链接和当天选择已经核对；详细段落未在旧数据文件中找到时，不补造实验数字。",
          limits: "这一回退卡片表示旧站正文源缺失，不代表论文缺少内容。",
          relation: "它保留在正确的每日位置，便于继续与同日论文和整月研究线索比较。",
        };
      }),
    }));
    ...
    <div className="reading-site">
      <header className="masthead">
        <div>
          ...
          <h1>AI 编译器论文阅读</h1>
          <p>2026 年 8 月 1 日至 8 月 26 日，每天按当日历史简报独立组织。</p>
        </div>
        <div className="masthead-meta"><span>26 个阅读日</span><span>{allPapers.length} 篇主线论文</span><span>更新至 2026-08-26</span></div>
      </header>

      <section className="intro-panel">
        <div>
          <h2>这次修订解决什么</h2>
          <p>旧页面混用了论文提交日期、网页添加日期和实际阅读日期，导致多篇论文被放到错误的日子，一些真正的每日主线反而没有显示。现在页面以历史简报中的<strong>阅读日期</strong>为唯一组织依据。</p>
          <p>每个日期使用同一结构。论文的核心机制、具体例子、实验证据和边界优先展示；不再按制作批次区分内容，也不使用外部图片。</p>
        </div>
        <div className="intro-facts">
          <div><strong>8 月 1—9 日</strong><span>根据早期网页、研究记录和周内关系恢复</span></div>
          <div><strong>8 月 10—26 日</strong><span>按 Daily AI Compiler 历史日更重新映射</span></div>
          <div><strong>横向分析</strong><span>统一连接新硬件、测试、Agent 搜索与性能诊断</span></div>
          {missingCount > 0 ? <div className="warning-fact"><strong>{missingCount} 篇旧正文待继续恢复</strong><span>论文身份和日期已修正，页面未补造缺失实验数字</span></div> : null}
        </div>
      </section>
      ...
      <div className="toolbar">
        ...
        <div className="track-filters" aria-label="研究方向筛选">
          {["全部", ...Object.keys(trackDescriptions)].map((name) => <button key={name} className={track === name ? "active" : ""} onClick={() => setTrack(name)} type="button">{name}</button>)}
        </div>
      </div>
      ...
      <div className="reading-layout">
        <aside className="date-index">
          <h2>每日索引</h2>
          <nav>
            {days.map((day) => <a href={`#day-${day.date}`} key={day.date}><time>{day.date.slice(5)}</time><span>{day.title}</span><em>{day.papers.length === 0 ? "周度收束" : `${day.papers.length} 篇`}</em></a>)}
          </nav>
        ...
        <main className="reading-main">
          {visibleDays.map((day) => (
            <section className="daily-reading" id={`day-${day.date}`} key={day.date}>
              <header className="day-header">
                ...
                <span className="paper-count">{day.papers.length === 0 ? "当日无新增主线论文" : `${day.papers.length} 篇主线论文`}</span>
              </header>

              <div className="daily-signal"><strong>今日研究信号</strong><p>{day.signal}</p></div>

              {day.papers.length > 0 ? day.papers.map((paper) => <PaperArticle key={`${day.date}-${paper.id}`} paper={paper} />) : (
                <div className="empty-day"><h3>当天没有为了凑数量加入低相关论文</h3><p>本日用于收束第一周的关系：机器语义进入编译器，生成空间由语法和合同约束，benchmark 与 Oracle 自身也需要审计。</p></div>
              )}

              <div className="daily-comparison">
                <h3>当日横向判断</h3>
                <p>{day.papers.length > 1 ? `${day.papers[0].zhTitle} 与 ${day.papers[1].zhTitle} 处理的是同一研究链条中的不同环节。` : day.papers.length === 1 ? `${day.papers[0].zhTitle} 是当天最值得保留的主线。` : "当天没有新增主线论文，重点是整理本周已经出现的方法关系。"} 当日已形成可继续使用的研究产物：<strong>{day.artifact}</strong>。</p>
              </div>
            </section>
          ))}
          ...
          <section className="monthly-map" id="monthly-map">
            <header><div className="eyebrow">2026 年 8 月横向分析</div><h2>从“让 Agent 写代码”走向“让编译器管理约束、证据和搜索”</h2><p>8 月 1 日至 26 日的论文可以归纳为四条相互连接的线。它们不是独立热门词，而是逐步构成一套可靠的自动编译系统。</p></header>

            <div className="track-overview">{trackStats.map((item) => <article key={item.name}><strong>{item.count}</strong><h3>{item.name}</h3><p>{item.description}</p></article>)}</div>

            <div className="relation-list">{relationCards.map((card) => <article key={card.title}><h3>{card.title}</h3><p className="relation-chain">{card.chain}</p><p>{card.text}</p></article>)}</div>

            <div className="month-judgments">
              <h3>本月最重要的四个判断</h3>
              <ol>
                <li><strong>新硬件支持的稳定知识应该进入编译器。</strong>机器语义、合法性、同步和资源规则不应由每个 Agent 在每个任务中重新推理。</li>
                <li><strong>正确性需要多层证据。</strong>语法通过、随机测试、数学等价、浮点数值合同、真实框架行为和硬件资源检查分别回答不同问题。</li>
                <li><strong>搜索机制必须接受固定预算对照。</strong>多 Agent、归档、反馈和长期记忆真正有价值的前提，是在相同编译与硬件测量预算下提高最终结果。</li>
                <li><strong>性能差距必须先定位层级。</strong>当 heuristic 已经接近形式目标最优时，继续强化搜索可能无效；问题可能位于成本模型、下游 DMA、tiling 或最终机器码。</li>
              </ol>
            </div>
          </section>
        </main>
