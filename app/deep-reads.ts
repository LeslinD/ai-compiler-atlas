...
export const recentDeepReads: DeepRead[] = [
  ...
  {
    id: "tensor-seeks-layout",
    date: "2026-08-25",
    title: "Tensor Seeks Layout: Formalizing Layout Selection for ML Compilers",
    chineseTitle: "Tensor Seeks Layout：把布局选择写成全图优化问题",
    arxiv: "2608.21555",
    source: "https://arxiv.org/abs/2608.21555",
    authors: "Clemens Eisenhofer、Yuwen Jia、Daniel Kroening、Sergey Pupyrev",
    version: "arXiv v1 · 2026-08-21（8 月 25 日阅读）",
    sections: [
      {
        heading: "它解决的问题",
        paragraphs: [
          "同一个张量布局不会在整张图里一直是最合适的。矩阵乘可能偏好一种存储顺序，紧随其后的规约偏好另一种；一个生产者还可能同时服务多个消费者。若每个算子各自选最快的布局，边上不断插入的 transpose、reformat 或 DMA 往往会抵消局部收益。",
          "这篇论文把问题从“为某个算子挑一个好格式”改写为“给数据流图上的每个张量挑布局，并决定哪里需要转换”。目标不是让每个节点单独最快，而是让算子执行与全图数据搬运的总成本最低。这也是它比常见经验传播规则更值得读的地方。",
        ],
      },
      {
        heading: "先分清：布局、分区与地址不是一件事",
        paragraphs: [
          "论文中的 layout 是张量的物理表示，随硬件而变。在 XLA 式 GPU 例子里，它可以是 minor-to-major 的维度顺序，例如 NCHW 或 NHWC；在 Trainium 这类片上 systolic array 例子里，它还可以是张量被哪一维分到片上分区。后者一旦不匹配，不能只靠改变循环顺序补救，通常要做真实的数据重排或 DMA。",
          "这与本地内存的地址分配应分开看。布局或分区先回答“一个元素按什么方式被组织、会落到哪类分区”；地址分配随后回答“这个已经选定表示的 buffer 在哪段本地内存、活多久、能否复用”。论文的形式化只优化前者，并把内存分配、调度、tiling 等列为会影响成本却未被完整联立的后续阶段。地址分配无法把不兼容的分区维度变成兼容；它只能在既定表示下安排位置和生命周期。",
        ],
      },
      {
        heading: "全图模型怎么写",
        paragraphs: [
          "把程序看成有向无环的数据流图。每个张量 t 有一个有限的可选布局集合 L(t)。一个算子的输入、输出布局组合有执行代价；若组合在硬件上无法实现，就把它的代价写成无穷大。生产者的布局与某个消费者需要的布局不同时，再付一次转换代价。",
          "总目标由两部分相加：所有算子的执行代价，加上每个张量转换到其不同消费者布局的代价。同一个生产者若有两个消费者都需要同一种转换后布局，转换可以共享，而不是按边重复计算。这个细节让 fan-out 图和链式图有不同最优解，也解释了为何只看相邻两个算子仍不够。",
        ],
      },
      {
        heading: "一个小例子：局部最快为什么会输",
        paragraphs: [
          "论文先给出矩阵乘—规约—加法的链。若所有张量保持 row-major，矩阵乘很便宜，但规约要跨 stride 访问；若矩阵乘输入改成 column-major，规约可以变得连续，却可能要求矩阵乘输出额外转置。哪种方案更好，取决于一次转置是否抵得过后续规约的收益，而不是单看矩阵乘或规约。",
          "另一种更容易踩坑的图是 convolution 接多个 BatchNorm，再汇入 Add。卷积可能喜欢 NHWC，BatchNorm 可能喜欢 NCHW。若每个 BatchNorm 都只选自己最快的 NCHW，系统会在它们之后分别插入转换；让 BatchNorm 暂时运行在次优布局反而可能让所有转换消失。这里真正共享的是 Add 之前的表示选择。",
        ],
      },
      {
        heading: "为什么不能指望一个贪心规则解决",
        paragraphs: [
          "作者证明了最优布局选择是 NP-hard：即使每个算子最多三个输入，问题仍可从 3SAT 归约而来。更强的结果是，即使图中除源节点外都只是二维矩阵乘、每个张量只有两种布局、转换成本只有 0 或 1、算子代价只有 0 或无穷大，问题仍是 NP-hard。",
          "这不是在说工程上一定要穷举。论文给出两条可执行路径：图的 treewidth 有界时，用动态规划可精确求解；一般图则编码成带权 MaxSAT，让现成求解器在同一个代价定义下找最优解。理论结果还提醒我们，不能给一般情形期待一个与最优值成固定比例的多项式近似保证；布局选择需要利用图结构、硬件限制或成本模型来缩小问题。",
        ],
      },
      {
        heading: "实验真正告诉了什么",
        paragraphs: [
          "作者把模型放进 AWS Trainium 的生产编译器，比较简单贪心、既有 rule-based 策略和 MaxSAT 求解器。简单启发式在部分工作负载上能造成最高约 5 倍的模型执行时间退化；在成本模型较准确的任务上，求解器能追平或超过既有规则策略。",
          "更有价值的反例也被保留了：在视觉和多模态等数据移动复杂的模型上，MaxSAT 已把论文定义的目标压到最优，实机却仍可能落后。作者报告不同算法在目标值上只差几个百分点时，硬件执行时间仍能相差最高约 4.6 倍；例如 MusicGen 中，规则策略的预测目标更差，却测得约 27.0 百万周期，而求解结果约为 38.5 百万周期。原因不是求解器没有找到解，而是布局决策后发生的 tiling、DMA 与其他下游行为没有被成本模型准确表示。",
        ],
      },
      {
        heading: "哪些结论不能直接外推",
        bullets: [
          "实验只在一个生产编译器和 AWS Trainium 上完成。GPU、TPU、移动端可以使用同一建模思路，但不能把具体加速幅度照搬过去。",
          "在某些 GPU 上，kernel 可以通过 loop interchange 或转置的 operand interface 吸收一部分布局不匹配；在 systolic array 的固定片上分区上，这种不匹配更可能变成真实搬运。硬件模型必须自己测量。",
          "论文把算子成本与转换成本相加，没有完整表示并行重叠、后续融合、tile 形状、buffer 分配和调度。它给出的是布局选择层的最优，不是完整编译流水线的最优。",
          "MaxSAT 适合用作基准、回归 oracle 或中小图的精确求解器；大型图仍需要分解、限制候选布局或结合启发式，否则求解开销本身会失控。",
        ],
      },
      {
        heading: "与已有编译器方法的关系",
        paragraphs: [
          "XLA 的 layout assignment、TVM 的期望布局与转换插入，已经在工程中处理过这个问题；它们多从局部有影响力的布局出发，再向前后传播并补 copy。SmartMem、NeoCPU 等也为特定移动 GPU 或 CPU 写过专门的传播规则。论文的贡献不是替换这些系统，而是给它们共同的代价对象和可比较的最优基准。",
          "与用强化学习联合选择图布局和循环变换的 ALT、用 ILP 优化布局变换的 VTensor 相比，这篇论文把可行性、算子代价和每张量共享转换写得更明确，并补上了难度界、treewidth 算法和 MaxSAT 编码。它特别适合拿来检查：某条规则到底是在逼近什么目标，还是只是在历史 workload 上碰巧有效。",
        ],
      },
      {
        heading: "和你的研究的关系",
        paragraphs: [
          "对 TileLang-TPU，layout 不能只写成 GPU 风格的索引映射。候选描述至少应区分 tile 形状、维度顺序、片上分区轴和所处 memory space；图级 pass 决定这些表示后，再让 buffer allocator 负责具体地址、生命周期和复用。每个布局转换都应有可测的 DMA 或重排代价，而不是只按算子次数计分。",
          "对 Tile for DSA，这篇论文给出了一条很实用的分层：先在图级选择逻辑布局和分区，再在 tile 级选择循环、bank 使用和搬运，再做局部内存地址安排。把三层混成一个 Layout 对象会让代价模型、合法性检查和调试日志都失去边界；把它们分开，才能指出某次失败究竟是图表示冲突、DMA 过多，还是 buffer 放不下。",
          "对 TileFuzz，可以专门生成 fan-out、fan-in、分区冲突和共享转换的微型图。小图用 MaxSAT 或 treewidth 动态规划产生可验证的最优答案；后端启发式若比它多出转换、选到不可行组合或随图形变化退化，就能得到比随机 kernel 更尖锐的回归用例。",
          "对 Agent-friendly kernel DSL，候选生成器应能读到显式的布局域、算子合法组合、转换方式和测得的代价，而不是只收到“把它调快”的目标。对于小子图，可把求解器结果当作决策校验；对于大图，记录启发式目标值、实际时间和误差，持续修正成本模型。对 LLM 处理 unsafe Rust，这篇论文较为间接，但同样提示了一个原则：把内存表示与合法性约束写进可检查的中间层，再让生成代码的系统在边界内工作。",
        ],
      },
    ],
  },
